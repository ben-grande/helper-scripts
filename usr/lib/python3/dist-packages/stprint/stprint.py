...
def exclude_pattern(original_pattern: str, negate_pattern: list[str]) -> str:
    ...
    >>> exclude_pattern(r"(30|31)", ["31"])
    '(?!(?:31))(30|31)'

    Regex in the negating pattern:
    >>> exclude_pattern(r"(38;5;[0-9])", ["38;5;\\d+"])
    '(?!(?:38;5;\\d+))(38;5;[0-9])'
    """
...
def get_color_pattern(
    ...
    *   Invalid SGR will lead to one of the following behaviors:
    ...
    >>> exclude_pattern = ["30", "4[0-7]", "38;5;\\d+", "38;2;0;\\d+;255"]
...
def gen_output(
    untrusted_text: str,
    colors: Optional[bool] = False,
    sgr_pattern: Union[str, Pattern[str]] = re.compile(r"(?!)"),
    ...
    >>> list(gen_output("\b\x1b[31m\x1b[m\x1b[2K", colors=True,
    ...                 sgr_pattern="(0|3[0-7])?m"))
    ...
    ...                 sgr_pattern=regex))
    """
    ## Avoid empty regex matching everything.
    if colors and not sgr_pattern:
        raise ValueError("colors is set but sgr_pattern is not")
...
def stprint(
    ...
    return "".join(
        list(
            gen_output(untrusted_text, colors=colors, sgr_pattern=sgr_pattern)
        )
    )
